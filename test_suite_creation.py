...
if __name__ == "__main__":
    ...
    parser.add_argument("--path", type=str, default=default_path, help="Path to generate the Test Directory")
    ...
    if BASE_DIR != default_path:
        print(f"WARNING: You have specified a custom path for the Test Directory: {BASE_DIR}. \n Ensure this is intentional to avoid data loss, as the directory will be cleared and recreated.")
        response = input("Enter 'Y' to confirm and wipe the directory, or any other key to exit: ").strip().upper()
        if response != 'Y':
